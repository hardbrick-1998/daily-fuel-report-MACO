...
def read_historical_safe(ttl_val=15): # <--- Tambahkan ttl_val=15 di sini
    """Mencoba baca database 3 kali berturut-turut jika sinyal putus."""
    for attempt in range(3):
        try:
            time.sleep(random.uniform(0.1, 1.0)) # <--- Tambahkan Jitter ini
            # ---> Ubah ttl=0 menjadi ttl=ttl_val di bawah ini
            df = conn.read(worksheet="HISTORICAL", dtype=str, ttl=ttl_val)
...
# Load Master Data
@st.cache_data(ttl=None) # <--- Ubah 600 menjadi None (Ingatan Permanen)
...
def generate_report_image(df_print, tanggal, shift, total_vol):
    ...
    try:
        df_limit_sheet = conn.read(worksheet="USAGE PER AREA", ttl=600) # <--- Ubah 0 jadi 600
...
def render_laporan_stock(tgl_laporan, shift_selected, tgl_pilih_indo):

    try:
        # Tambahkan dtype=str
        df_report = read_historical_safe(ttl_val=300) # <--- PASTIKAN ADA ANGKA 300 INI
    ...
    # Simpan state tabel aktif agar Fitur Hapus bisa membacanya
        st.session_state['data_tabel_aktif'] = df_filtered
    except Exception as e: 
        st.error(f"⚠️ Error memuat tabel: {e}")

# BUKA KEMBALI TAB INPUT AGAR TABEL MASUK KE DALAMNYA
with tab_input:
    render_laporan_stock(tgl_laporan, shift_selected, tgl_pilih_indo)
...
def render_dashboard_analytics():
    try:
        # Tambahkan dtype=str
        df_dash = read_historical_safe(ttl_val=300)
...
with st.sidebar.expander("🗑️ HAPUS DATA (KHUSUS ADMIN / PENGAWAS)"):
    ...
    df_filtered_hapus = st.session_state.get('data_tabel_aktif', pd.DataFrame())

    if not df_filtered_hapus.empty:
        pilihan_hapus = []
        mapping_index = {} 
        
        for idx, row in df_filtered_hapus.iterrows():
